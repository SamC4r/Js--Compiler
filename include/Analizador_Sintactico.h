...
class AnalizadorSintactico {
    ...
    map<string,string> numero_accion_semantica = {
        ...
        {"{xx.yy}","Z -> {zona_declaracion=true} B Z1     {Z.tipo = if (B.tipo=tipo_ok AND (Z1.tipo = vacio OR Z1.tipo = tipo_ok)) then tipo_ok else tipo_error }"},
        {"{xx.yy}","Z -> F Z1   {Z.tipo = if (F.tipo=tipo_ok AND (Z1.tipo = vacio OR Z1.tipo = tipo_ok) then tipo_ok else tipo_error}"},
        {"{xx.yy}","Z -> lambda {Z.tipo=vacio}"},
        {"{xx.yy}","B -> var T id ; {InsertaTipo(id.pos,T.tipo), InsertaDesp(ts,*dl + T.ancho), B.tipo=tipo_ok}"},
        {"{xx.yy}","B -> if ( E ) I  {B.tipo=if(E.tipo!=logico then error(\"Condcion del if incorrecta: solo se pueden comprobar condiciones logicas\")) else I.tipo}"},
        {"{xx.yy}","B -> {zona_declaracion=false} S {B.tipo=S.tipo}"},
        {"{xx.yy}","T -> int     {T.tipo=ent, T.ancho=4}"},
        {"{xx.yy}","T -> boolean    {T.tipo=logico, T.ancho = 1}"},
        {"{xx.yy}","T -> string  {T.tipo=cadena, T.ancho=64}"},
        {"{xx.yy}","E -> R N  {E.tipo = if (R.tipo = entero AND N.tipo=logico) then logico else if (N.tipo = vacio) then R.tipo else error (“solo se pueden comparar enteros”)}"},
        {"{xx.yy}","N -> > R N1   {N.tipo = if ( R.tipo = entero AND (N1.tipo=logico OR N1.tipo = vacio)) then logico else error(“solo se pueden comparar enteros”)}"},
        {"{xx.yy}","N -> lambda  {N.tipo=vacio}"},
        {"{xx.yy}","R -> O P {R.tipo = if (P.tipo = vacio) then O.tipo else if (O.tipo = P.tipo = entero) then entero else error (“Solo se pueden comparar numeros”)"},
        {"{xx.yy}","P -> - O P1   {P.tipo = if ((P1.tipo = vacio AND O.tipo=entero) OR O.tipo = P1.tipo = entero) then entero else error(“solo se puede restar entre numeros enteros”)"},
        {"{xx.yy}","P -> lambda  {P.tipo=vacio}"},
        {"{xx.yy}","O ->  M Y  {O.tipo = if (Y.tipo = vacio then M.tipo else if M.tipo = Y.tipo = entero) then entero else error(“solo se puede calcular el resto entre numeros enteros”)"},
        {"{xx.yy}","Y -> % M Y1   { Y.tipo = if (M.tipo = Y1.tipo = entero) then entero else error(“solo se puede calcular el resto entre numeros enteros”) }"},
        {"{xx.yy}","Y -> lambda  {Y.tipo=vacio}"},
        {"{xx.yy}","M -> - M1     {M.tipo = if (M1.tipo != entero) then error(“solo se puede hacer aritmetica sobre numeros”) else entero}"},
        {"{xx.yy}","M -> ! M1     {M.tipo = if(M1.tipo != logico) then error(“Solo se pueden negar tipos logicos”) else logico}"},
        {"{xx.yy}","M -> id V      {M.tipo = if (V.tipo = vacio then BuscaTipo(id.pos)) else if BuscaTipoTS(id.pos) != V.tipo->T then error(“funcion llamada con parametros incorrectos, esperados %d” else BuscaTipoTS(id.pos)) "},
        {"{xx.yy}","M -> ( E )   {M.tipo=E.tipo}"},
        {"{xx.yy}","M -> constanteEntera   {M.tipo=entero}"},
        {"{xx.yy}","M -> cadena  {M.tipo=cadena}"},
        {"{xx.yy}","M -> -- id {M.tipo = if (BuscaTipo(id.pos) != entero) then error(“solo se pueden predecrementar numeros”) else entero}"},
        {"{xx.yy}","V -> ( L )  {V.tipo=L.tipo}   "},
        {"{xx.yy}","V -> lambda  {V.tipo=vacio}"},
        {"{xx.yy}","L -> E Q  {L.tipo = if (Q.tipo != vacio) then E.tipo x K.tipo else E.tipo }"},
        {"{xx.yy}","L -> lambda  {L.tipo=vacio}"},
        {"{xx.yy}","Q -> , E Q1    {Q.tipo = if (Q1.tipo != vacio) then E.tipo x Q1.tipo else E.tipo}"},
        {"{xx.yy}","Q -> lambda  {Q.tipo=vacio}"},
        {"{xx.yy}","I -> S {I.tipo = S.tipo}"},
        {"{xx.yy}","I -> { C } J"},
        {"{xx.yy}","J -> else { C }    "},
        ...
        {"{xx.yy}","F -> function H {ts = crearTSLocal(), dl = creaDespLocal() despLocal=0, zona_decl=true}  id ( A ) {InsertarTipoTSGlobal(id.pos,A.tipo -> H.tipo} { C }  {F.tipo=C.tipo}"},
